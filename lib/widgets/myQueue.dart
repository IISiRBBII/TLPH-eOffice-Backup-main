...
class myQueueCalendarWidget extends StatefulWidget {
  final List<queueModel> lGetQueue;
  final String headerText;

  const myQueueCalendarWidget(
      {Key? key, required this.lGetQueue, required this.headerText})
      : super(key: key);

  @override
  State<myQueueCalendarWidget> createState() => _CalendarWidgetState();
}

class _CalendarWidgetState extends State<myQueueCalendarWidget> {
  @override
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Padding(
                        padding: EdgeInsets.only(top: 4, bottom: 4),
                        child: Text('เลขห้อง : ${appointment.unitId}'),
                      ),
                    ),
                    Container(
                      width: 120,
                      ...
                      child: Padding(
                        padding: EdgeInsets.only(top: 4, bottom: 4),
                        child: Text('ห้อง : ${appointment.unit}'),
                      ),
                    ),
                    Container(
                      width: 120,
...
CalendarDataSource _lGetCalendarDataSource(List<queueModel> lGetQueue) {
  ...
  for (var qu in lGetQueue) {
    if (qu.type == '') {
      // กรองนัดหมายที่มี type เป็น 'room' เท่านั้น
      ...
      appointments.add(Appointment(
        ...
        unitId: qu.unitId,
        unit: qu.unit,
        boxColor: Colors.green, // เปลี่ยนสีตามต้องการ
      ));
    }
  }
